    ...
    if (lookupCacheInfo) {
        ...
        if (([[lookupCacheInfo expiryDate] compare:[NSDate new]] == NSOrderedAscending)) {
            ...
            switch ([lookupCacheInfo fileType]) {
                ...
                case DictionaryType: {
                    NSDictionary *returnDictionary = (NSDictionary *)
        ...
        //  Data does not exist in the lookup, most likely not available in the
        //  persistent storage
        return nil;
    }
...
- (FileTypeEnum)extractFileType:(NSString *)url {
    ...
    if (([[extension uppercaseString] isEqualToString:@"JPEG"]) ||
        ([[extension uppercaseString] isEqualToString:@"JPG"]))
    {
...
- (NSString *)encodedString:(NSString *)string {
    ...
    CFStringRef escapedString = CFURLCreateStringByAddingPercentEscapes(
                                                                        kCFAllocatorDefault, (__bridge CFStringRef)string, NULL, charsToEscape,
                                                                        kCFStringEncodingUTF8);
    return (__bridge_transfer NSString *)escapedString;
}

- (NSData *)encryptintoData:(NSObject *)anObject
                   dataType:(FileTypeEnum)dataTypeValue {
    NSData *data;
...
- (void)addToCacheLookup:(NSString *)key
                fileType:(FileTypeEnum)fileType
              expiryDate:(NSDate *)date {
    
    NSManagedObjectContext *context = [self managedObjectContext];
    
    CacheStatus *cacheInformation =[NSEntityDescription insertNewObjectForEntityForName:@"CacheStatus" inManagedObjectContext:context];
...
- (NSURL *)applicationDocumentsDirectory {
    ...
    // code uses a directory named "test.cachelib" in the application's documents
    // directory.
